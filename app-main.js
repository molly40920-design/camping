...
function App() {
  ...
  // 個人行前準備：是否已匯入過（控制預設顯示匯入還是清單）
  const [hasImported, setHasImported] = useState(() => {
    const key = 'campsync_hasImported_' + (localStorage.getItem('campsync_roomId') || '') + '_' + (localStorage.getItem('campsync_userName') || '');
  ...
  const getHasImportedKey = useCallback(() => {
    if (!roomId || !userName) return null;
    return 'campsync_hasImported_' + roomId.toLowerCase() + '_' + userName;
  }, [roomId, userName]);
  ...
  const loadPrepItems = useCallback(() => {
    ...
    // 同步 hasImported 狀態
    const hKey = getHasImportedKey();
    if (hKey) { setHasImported(localStorage.getItem(hKey) === 'true') }
  }, [getPrepKey, getHasImportedKey]);
  ...
  const handleBatchImport = async (sel) => {
    ...
    // 標記已匯入
    const hKey = getHasImportedKey();
    if (hKey) { localStorage.setItem(hKey, 'true'); setHasImported(true); }
  ...
  const saveSharedItem = async (itemData) => {
    ...
    if (isSavingRef.current) return; // 防連點
  ...
  const handleSelectAll = () => {
    ...
    const allSelected = currentItemIds.length > 0 && currentItemIds.every(id => selectedIds.includes(id));
    if (allSelected) {
      // 取消全選：移除當前 tab 的 id
      ...
      setSelectedIds(prev => {
        ...
        currentItemIds.forEach(id => s.add(id));
        return Array.from(s);
      });
    }
  };
  ...
  // 防連點 Loading Overlay - 營地內新增/刪除/編輯時顯示
  ...
  const currentTabItems = currentTab === 'meals'
    ? sharedItems.filter(i => i.type === 'meal')
    : sharedItems.filter(i => i.type === 'equipment');
  const currentTabItemIds = currentTabItems.map(i => i.id);
  const isAllSelected = currentTabItemIds.length > 0 && currentTabItemIds.every(id => selectedIds.includes(id));
      ...
      e('main', { className: "flex-1 overflow-y-auto p-4 pb-32 relative" },
        ...
        !hasEnteredCamp && e('div', { className: "space-y-6 anim-fadeIn" },
          ...
          e('div', { className: "space-y-4" },
            ...
            // 如果還沒匯入過且清單為空，顯示匯入引導
            (!hasImported && prepItems.length === 0)
              ? e('div', { className: "flex flex-col items-center justify-center py-12 text-center px-4 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200" },
                e(Icon, { name: 'listChecks', size: 48, className: "text-emerald-300 mb-4" }),
                e('p', { className: "text-slate-500 text-sm mb-1" }, '還沒有行李清單？'),
                e('p', { className: "text-slate-400 text-xs mb-5" }, '點擊下方按鈕快速匯入常用物品，也可以自行新增'),
                e('button', { onClick: () => setIsImportModalOpen(true), className: "bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 px-6 rounded-xl shadow-md transition-colors flex items-center gap-2" },
                  e(Icon, { name: 'listChecks', size: 18 }), ' 快速匯入清單'
                )
              )
        ...
        hasEnteredCamp && e('div', { className: "anim-fadeIn" },
          // 全選工具列（公共裝備 / 伙食計劃才顯示）
          currentTab !== 'settlement' && currentTabItems.length > 0 && e('div', { className: "flex items-center justify-between mb-3 bg-slate-50 px-3 py-2 rounded-xl border border-slate-100" },
            e('label', { className: "flex items-center gap-2 cursor-pointer select-none" },
              e('input', { type: 'checkbox', checked: isAllSelected, onChange: handleSelectAll, className: "w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer" }),
              e('span', { className: "text-sm text-slate-600 font-medium" }, isAllSelected ? '取消全選' : '全選')
            ),
            selectedIds.length > 0 && e('span', { className: "text-xs text-slate-400" }, '已選 ' + selectedIds.length + ' 項')
          ),
